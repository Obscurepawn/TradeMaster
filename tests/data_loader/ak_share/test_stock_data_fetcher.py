...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', '..', 'data_loader', 'ak_share'))
...
class TestStockDataFetcher(unittest.TestCase):
    ...
    def test_calculate_technical_indicators(self):
        ...
        expected_columns = [
            ...
            'RSI', 'Middle_Band', 'Upper_Band', 'Lower_Band',
            'VMA5', 'VMA10', 'K', 'D', 'J', 'BIAS5', 'BIAS10'
        ]
    ...
    def test_get_zh_a_stock_history_exception(self, mock_sleep, mock_stock_hist):
        ...
        self.assertIn("Failed to get stock historical data after max retries", str(context.exception))
        mock_stock_hist.assert_called_once()
        mock_sleep.assert_called_once()
    ...
    def test_get_zh_a_stock_histories_success(self, mock_tqdm, mock_get_history):
        ...
        try:
            ...
            with patch('data_loader.ak_share.impl.get_config_value') as mock_get_config:
                mock_get_config.return_value = temp_dir  # Use temp directory instead
                # Call the function
                get_zh_a_stock_histories(
    ...
    @patch('data_loader.ak_share.impl.get_zh_a_stock_history')
    @patch('data_loader.ak_share.impl.tqdm')
    def test_get_zh_a_stock_histories_with_none_result(self, mock_tqdm, mock_get_history):
    ...
    @patch('data_loader.ak_share.impl.get_zh_a_stock_history')
    @patch('data_loader.ak_share.impl.tqdm')
    def test_get_zh_a_stock_histories_exception_handling(self, mock_tqdm, mock_get_history):
